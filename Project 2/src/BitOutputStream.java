...
public class BitOutputStream extends OutputStream {
    ...
    /**
     * Converts the given character into a format that prints well on the
     * screen. For most characters, this just means enclosing it in ' ' marks.
     * For special characters such as \n, replaces them with a \ and an n. Also
     * replaces the special EOF character with the string "EOF".
     *
     * @param ch
     ...
     *            the target output stream to write.
     * @param bitMode
     *            true to write bits at a time; false to write ASCII characters
    ...
    /**
     * Flushes the buffer. If numDigits < BYTE_SIZE, this will effectively pad
     * the output with extra 0s, so this should be called only when numDigits ==
     * BYTE_SIZE or when we are closing the output.
     *
     * @throws IOException
     *             if the bits cannot be written.
     */
    @Override
     ...
     * students should not call this method. This is a hack so that a
     * BitInputStream knows when it is out of bits.
     *
    ...
    /**
     * Sets whether this stream is in real 'bit mode', writing a bit to the file
     * for each 0 or 1 written in writeBit (as described under inBitMode).
     ...
     *
     * @param bitMode
     *            true to use bit mode, false to use character mode.
     */
     ...
     * encoding for the end of a file (EOF); students should not call this
     * method.
     *
     * @param eofEncoding
     *            the EOF encoding to use
     * @throws NullPointerException
    ...
    public void write(int b) throws IOException {
        if (b == EOF || seenEOF) {
            ...
            if (DEBUG) {
                System.out.println(
                        "  ** BitOutputStream write: " + b + " (" + toPrintable((char) b) + ")");
            }
            output.write(b);
        }
    }
     ...
     *            the byte value to write.
     * @param offset
     *            the beginning index to use.
     * @param length
     *            the number of indexes to write.
     * @throws IOException
     *             if the bytes cannot be written.
     * @throws NullPointerException
     *             if bytes is null.
    ...
    public void write(byte[] bytes, int offset, int length) throws IOException {
        if (seenEOF) {
            ...
            if (DEBUG) {
                System.out.println("  ** BitOutputStream write: " + java.util.Arrays.toString(bytes)
                        + ", " + offset + ", " + length);
            }
    ...
    public void writeBits(String bits) throws IOException {
        for (int i = 0; i < bits.length(); i++) {
            ...
            if (ch == '0' || ch == 0) {
                ...
                throw new IllegalArgumentException("Illegal bit value '" + ch + "' at index " + i
                        + " of string \"" + bits + "\"");
            }
        }
    }
    ...
    /**
     * Runs when the object is garbage collected / program shuts down. Included
